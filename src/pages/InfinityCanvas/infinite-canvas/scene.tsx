  ...
  React.useEffect(() => {
    ...
    const onMouseUp = (e: MouseEvent) => {
      ...
      const dist = Math.sqrt(
        Math.pow(e.clientX - (s.mouseDragStart?.x || 0), 2) +
          Math.pow(e.clientY - (s.mouseDragStart?.y || 0), 2),
      );
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (touches.length > 0 && s.touchStartPos) {
        const dist = Math.sqrt(
          Math.pow(touches[0].clientX - s.touchStartPos.x, 2) +
            Math.pow(touches[0].clientY - s.touchStartPos.y, 2),
        );
        if (dist > 10) {
          s.isFocusing = false;
          s.focusTarget = null;
